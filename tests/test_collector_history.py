...
from metrika_lead_pipeline.models import NormalizedMetrikaData, PageFact, SignalFinding, Recommendation, SourceFact
...
class FakeTransport:
    ...
    def get(self, url: str, headers: dict[str, str], params: dict[str, object], timeout: float) -> dict[str, object]:
        ...
        if dimensions == "ym:s:startURL":
            return {"data": [{"dimensions": [{"name": "/cars/a-vs-b"}], "metrics": [150]}]}
        if dimensions == "ym:s:lastsignTrafficSource":
            return {"data": [{"dimensions": [{"name": "Search engine traffic"}], "metrics": [120]}]}
        if dimensions == "ym:s:startURL,ym:s:lastsignTrafficSource":
            return {"data": [
                {"dimensions": [{"name": "/cars/a-vs-b"}, {"name": "Recommendation system traffic"}], "metrics": [90]},
                {"dimensions": [{"name": "/cars/a-vs-b"}, {"name": "Search engine traffic"}], "metrics": [30]},
            ]}
...
def test_client_uses_cache_and_typed_collection(tmp_path: Path) -> None:
    ...
    assert data.pages[0].traffic_sources == {"Recommendation system traffic": 90, "Search engine traffic": 30}
    assert data.pages[0].discover_share == 0.75
    assert data.pages[0].search_traffic_share == 0.25
...
def test_merge_does_not_copy_global_sources_to_pages() -> None:
    ...
    data = normalizer.merge(
        ...
        entry_urls=set(),
        sources=[SourceFact(source="Recommendation system traffic", visits=100)],
        visits=[],
        search_queries=[],
        goals=[],
        devices=[],
    ...
    assert data.sources[0].source == "Recommendation system traffic"
    assert data.pages[0].traffic_sources == {}
    assert data.pages[0].search_traffic_share is None
    assert data.pages[0].discover_share is None


def test_normalizer_merges_url_level_sources_and_shares() -> None:
    normalizer = MetrikaNormalizer()
    page_sources = normalizer.normalize_page_sources({"data": [
        {"dimensions": [{"name": "/u"}, {"name": "Search engine traffic"}], "metrics": [40]},
        {"dimensions": [{"name": "/u"}, {"name": "Recommendation system traffic"}], "metrics": [30]},
        {"dimensions": [{"name": "/u"}, {"name": "Internal traffic"}], "metrics": [20]},
        {"dimensions": [{"name": "/u"}, {"name": "Social network traffic"}], "metrics": [10]},
    ]})
    data = normalizer.merge(
        pages=[PageFact(url="/u", title="Title")],
        entry_urls=set(),
        sources=[SourceFact(source="Direct traffic", visits=999)],
        visits=[],
        search_queries=[],
        goals=[],
        devices=[],
    ...
    page = data.pages[0]
    assert page.traffic_sources == {
        "Search engine traffic": 40,
        "Recommendation system traffic": 30,
        "Internal traffic": 20,
        "Social network traffic": 10,
    }
    assert page.search_traffic_share == 0.4
    assert page.discover_share == 0.3
    assert page.internal_share == 0.2
    assert page.social_share == 0.1
