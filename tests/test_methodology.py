...
def test_recommendation_system_price_page_routes_to_ab_test_and_blocks_form(tmp_path: Path) -> None:
    rec = _rec(PageFact(
        url="/cars/model-a-price",
        title="Model A цена комплектация тест-драйв",
        pageviews=5000,
        visitors=3000,
        traffic_sources={"Recommendation system traffic": 4500, "Search engine traffic": 500},
        discover_share=0.9,
        search_traffic_share=0.1,
    ))
    assert rec.traffic_context == "discover"
    assert rec.stage_hypothesis != "lower"
    assert rec.recommendation == "run_discriminating_test"
    assert rec.experiment_type == "ab_test_early_vs_commitment"
    assert rec.form_allowed is False
    run_pipeline_from_facts([PageFact(
        url="/cars/model-a-price",
        title="Model A цена комплектация тест-драйв",
        pageviews=5000,
        visitors=3000,
    ...
    payload = json.loads((tmp_path / "decision_log.json").read_text(encoding="utf-8"))
    decision = payload[0] if isinstance(payload, list) else payload["records"][0]
    assert decision["traffic_context"] == "discover"
