...
from src.history.comparator import RunComparator, write_changes_report
from src.history.storage import HistoryStorage
from src.models import NormalizedMetrikaData
from src.pipeline.runner import run_pipeline_from_facts
...
def collect_and_analyze(last_days: int | None = None, today: bool = False, yesterday: bool = False, month: str | None = None, date_from: str | None = None, date_to: str | None = None, region: str | None = None, brand: str | None = None, category: str | None = None, output: Path = Path("reports"), config_path: Path = Path("src/config/config.yaml"), collector: MetrikaReportCollector | None = None) -> dict[str, Any]:
...
def run_normalized(normalized: NormalizedMetrikaData, date_from: str, date_to: str, output: Path, config_path: Path = Path("src/config/config.yaml")) -> dict[str, Any]:
    ...
    snapshot = {"run_id": run_id, "created_at": datetime.now().isoformat(), "period": {"from": date_from, "to": date_to}, "config_version": cfg.version, "rules_version": load_yaml(Path("src/config/rules.yaml")).get("version", ""), "signals_version": load_yaml(Path("src/config/signals.yaml")).get("version", ""), "pages": [p.model_dump() for p in pages], "signals": [s.model_dump() for s in signals], "recommendations": [r.model_dump() for r in recommendations], "decision_log": [d.model_dump() for d in decisions], "limitations": normalized.limitations}
