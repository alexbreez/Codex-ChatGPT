...
class MetrikaNormalizer:
    ...
    def normalize_page_sources(self, payload: dict[str, Any]) -> list[SourceFact]:
        return [
            SourceFact(url=_dim(row, 0), source=_dim(row, 1), visits=int(_metric(row, 0)), raw=row)
            for row in payload.get("data", [])
            if _dim(row, 0) and _dim(row, 1)
        ]
    ...
    def normalize_regions(self, payload: dict[str, Any]) -> list[RegionFact]:
        ...
        for row in payload.get("data", []):
            country = _dim(row, 0)
            area = _dim(row, 1)
            label = f"{country} / {area}" if area else country
            regions.append(RegionFact(region=label, country=country, area=area, visits=int(_metric(row, 0)), raw=row))
        return regions
    ...
    def merge(self, pages: list[PageFact], entry_urls: set[str], sources: list[SourceFact], visits: list[VisitFact], search_queries: list[SearchQueryFact], goals: list[GoalFact], devices: list[DeviceFact], regions: list[RegionFact], limitations: list[str], page_sources: list[SourceFact] | None = None) -> NormalizedMetrikaData:
        ...
        for source in page_sources or []:
            if not source.url:
                continue
            totals = page_source_totals.setdefault(source.url, {})
            totals[source.source] = totals.get(source.source, 0) + source.visits
        for page in pages:
            page.is_entry_page = page.url in entry_urls
            if page.url in page_source_totals:
                _attach_page_sources(page, page_source_totals[page.url])
...
def _attach_page_sources(page: PageFact, sources: dict[str, int]) -> None:
    page.traffic_sources = dict(sources)
    total = sum(v for v in sources.values() if v > 0)
    if total <= 0:
        return
    page.search_traffic_share = _source_share(sources, total, "Search engine traffic")
    page.discover_share = _source_share(sources, total, "Recommendation system traffic")
    page.internal_share = _source_share(sources, total, "Internal traffic")
    page.social_share = _source_share(sources, total, "Social network traffic")


def _source_share(sources: dict[str, int], total: int, canonical_name: str) -> float:
    value = 0
    canonical = canonical_name.lower()
    for source, visits in sources.items():
        if source.lower() == canonical:
            value += visits
