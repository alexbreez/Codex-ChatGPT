...
def run_normalized(normalized: NormalizedMetrikaData, date_from: str, date_to: str, output: Path, config_path: Path = default_config_path("config.yaml")) -> dict[str, Any]:
    ...
    limits = cfg.outputs
    max_snapshot_pages = int(limits.get("max_snapshot_pages", len(pages)))
    max_snapshot_signals = int(limits.get("max_snapshot_signals", len(signals)))
    max_snapshot_recommendations = int(limits.get("max_snapshot_recommendations", len(recommendations)))
    max_snapshot_decisions = int(limits.get("max_snapshot_decisions", len(decisions)))
    truncated = _snapshot_truncation({"pages": (min(max_snapshot_pages, len(pages)), len(pages)), "signals": (min(max_snapshot_signals, len(signals)), len(signals)), "recommendations": (min(max_snapshot_recommendations, len(recommendations)), len(recommendations)), "decision_log": (min(max_snapshot_decisions, len(decisions)), len(decisions))})
    snapshot = {"run_id": run_id, "created_at": datetime.now().isoformat(), "period": {"from": date_from, "to": date_to}, "config_version": cfg.version, "rules_version": load_yaml(default_config_path("rules.yaml")).get("version", ""), "signals_version": load_yaml(default_config_path("signals.yaml")).get("version", ""), "pages": [p.model_dump() for p in pages[:max_snapshot_pages]], "signals": [s.model_dump() for s in signals[:max_snapshot_signals]], "recommendations": [r.model_dump() for r in recommendations[:max_snapshot_recommendations]], "decision_log": [d.model_dump() for d in decisions[:max_snapshot_decisions]], "limitations": normalized.limitations, "truncated": truncated}
