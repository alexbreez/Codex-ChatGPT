...
def _build_decisions(pages: list[PageFact], recs: list[Recommendation], evals: dict[str, list[RuleEvaluation]], config_version: str, signal_version: str, rules_version: str, global_limitations: list[str]) -> list[DecisionRecord]:
    ...
    for page in pages:
        ...
        records.append(DecisionRecord(
            ...
            recommendations=[rec.recommendation] if rec.status != "Недостаточно данных" else [], limitations=rec.limitations + global_limitations,
            run_id=run_id, url=rec.url, page_role=rec.page_role, job_hypothesis=rec.job_hypothesis,
            stage_hypothesis=rec.stage_hypothesis, stage_confidence=rec.stage_confidence,
            scores={
            ...
            purchase_signals=rec.purchase_signals, choice_signals=rec.choice_signals,
            risk_signals=rec.risk_signals, cold_news_signals=rec.cold_news_signals,
            triggered_constraints=rec.triggered_constraints, recommendation=rec.recommendation,
            recommended_cta_type=rec.recommended_cta_type, form_allowed=rec.form_allowed,
            form_prohibited=rec.form_prohibited, prohibition_reason=rec.prohibition_reason,
            rationale=rec.explanation, data_limitations=rec.data_limitations + global_limitations,
            manual_review_required=rec.manual_review_required, experiment_type=rec.experiment_type,
        ))
    return records
